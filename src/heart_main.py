...
from .models import Like, Outfit, Similar, User
...
def login(user: UserBase, db: Session = Depends(get_db)):
    ...
    if db_user is None or not pwd_context.verify(
        user.login_pwd, db_user.login_pwd
    ):  # type:ignore
...
@app.get("/heart")
async def heart(
    user_id: int, pagesize: int, offset: int, db: Session = Depends(get_db)
):
...
async def images(
    ...
    for outfit in outfits:
        try:
            (
                db.query(Like)
                .filter(Like.user_id == user_id, Like.outfit_id == outfit.outfit_id)
                .first()
            )
...
async def image(user_id: int, outfit_id: int, db: Session = Depends(get_db)):
    ...
    for similar_outfit in similar_outfits:
        img_url = (
            db.query(Outfit).filter(Outfit.outfit_id == similar_outfit).first().img_url
        )

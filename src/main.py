...
def images(response: Response,
    ...
    return {"outfits": db_outfits,"liked_outfits": liked_outfits, "pagesize":pagesize, "offset":offset, "is_last": is_last, "total_page_count": total_page_count}
...
def image(response: Response,
    ...
    if user_id==1:
        is_liked = db.query(Like).filter(Like.session_id == session_id).filter(Like.outfit_id == outfit_id).first()
    else:
        is_liked = db.query(Like).filter(Like.user_id == user_id).filter(Like.outfit_id == outfit_id).first()
    ...
    for similar in similar_outfits:
        similar_outfit = db.query(Outfit).filter(Outfit.outfit_id == similar.similar_outfit_id).first()
        similar_outfits_list.append(similar_outfit)
    
    return {"outfit": outfit, "similar_outfits": similar_outfits_list}
    
@app.get('/heart')
def heart(response: Response,
          pagesize : int,
          offset : int,
          user_id: int = Cookie(None),
          session_id: str= Cookie(None),
...
def heart(response: Response,
    ...
    if user_id==1:
        stmt = db.update(Like).where(Like.session_id == session_id).where(Like.outfit_id == outfit_id).values(is_deleted=True)
        db.execute(stmt)
        db.commit()
    else:
        stmt = db.update(Like).where(Like.user_id == user_id).where(Like.outfit_id == outfit_id).values(is_deleted=True)
        db.execute(stmt)
        db.commit()

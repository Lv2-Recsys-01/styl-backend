...
class UserBase(BaseModel):
    ...
    def login_id_check(cls, user_name: str):
        if not user_name.isalnum() or len(user_name) < 4 or len(user_name) > 12:
            raise HTTPException(
                status_code=500, detail="아이디는 4자리 이상 12자리 이하의 숫자 or 영문자입니다."
            )
        return user_name
    ...
    def login_pwd_check(cls, user_pwd: str):
        if not user_pwd.isalnum() or len(user_pwd) < 4 or len(user_pwd) > 20:
            raise HTTPException(
                status_code=500, detail="비밀번호는 4자리 이상 20자리 이하의 숫자 or 영문자입니다."
            )
        return user_pwd
